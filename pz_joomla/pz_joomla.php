...
class plgVmPaymentPz_joomla extends vmPSPlugin {
    ...
    function __construct(& $subject, $config) {
 	...
 	public function getVmPluginCreateTableSQL() {
		return $this->createTableSQL('Payment PZ JOOMLA Table');
    }
	...
	function plgVmConfirmedOrder($cart, $order) {		
		...
		if (empty($toid)) {
		    vmInfo(JText::_('VMPAYMENT_PZ_JOOMLA_MERCHANT_TOID_NOT_SET'));
		    return false;
		}
...
$country_value = $country_code[ShopFunctions::getCountryByID($order['details']['BT']->virtuemart_country_id, 'country_2_code')];
		...
		$dbValues['pz_joomla_custom'] = $return_context;
		$dbValues['billing_currency'] = $method->payment_currency;
		$dbValues['amount'] =(float) $totalInPaymentCurrency;
		$this->storePSPluginInternalData($dbValues);
	
		$url = $this->_getPZ_JOOMLA_UrlHttps($method);
		
		// add spin image
		$html = '<html><head><title>Redirection</title></head><body><div style="margin: auto; text-align: center;">';
		$html .= '<form action="' . "https://" . $url . '" method="post" name="vm_pz_joomla_form" >';
		$html.= '<input type="submit"  value="' . JText::_('VMPAYMENT_PZ_JOOMLA_REDIRECT_MESSAGE') . '" />';
		foreach ($post_variables as $name => $value) {
		    $html.= '<input type="hidden" style="" name="' . $name . '" value="' . htmlspecialchars($value) . '" />';
		}
		$html.= '</form></div>';
		$html.= ' <script type="text/javascript">';
		$html.= ' document.vm_pz_joomla_form.submit();';
    ...
    function plgVmOnPaymentResponseReceived(&$html) {
		...
		$this->_storePz_joomlaInternalData($method, $response, $virtuemart_order_id,$paymentTable->pz_joomlacustom);
		if($response['status']=='Y'){		
	...
	function _getPaymentResponseHtml($paymentTable, $payment_name, $response) {
		$html = '<table>' . "\n";
		$html .= $this->getHtmlRow('PZ_JOOMLA_PAYMENT_NAME', $payment_name);		
		if (!empty($paymentTable)) {
		    //$html .= $this->getHtmlRow('PZ_JOOMLA_ORDER_NUMBER', $paymentTable->order_number);
                    $html .= $this->getHtmlRow('PZ_JOOMLA_VIRTUEMART_ORDER_ID', $paymentTable->virtuemart_order_id);
		}
		
		
		$tot_amount = $response['amount']." INR";
		$html .= $this->getHtmlRow('PZ_JOOMLA_AMOUNT', $tot_amount);
		
	
		return $html;
    }
    
	function _storePz_joomlaInternalData($method, $response, $virtuemart_order_id,$custom) {
 	...
 	function plgVmOnUserPaymentCancel() {
		...
		VmInfo(Jtext::_('VMPAYMENT_PZ_JOOMLA_PAYMENT_CANCELLED'));
		$session = JFactory::getSession();
		$return_context = $session->getId();
		if (strcmp($paymentTable->pz_joomla_custom, $return_context) === 0) {
	...
	function plgVmOnShowOrderBEPayment($virtuemart_order_id, $payment_method_id) {
		...
		if (!($paymentTable = $this->_getPz_joomlaInternalData($virtuemart_order_id) )) {
		    // JError::raiseWarning(500, $db->getErrorMsg());
		    return '';
		}
    ...
    function _getPz_joomlaInternalData($virtuemart_order_id, $order_number = '') {
		$db = JFactory::getDBO();
	...
	function _getPZ_JOOMLA_UrlHttps($method) {
